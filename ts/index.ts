...
async function loadDataIncremental() {
  ...
  const loadVehicles = fetchWithTimeout("data/vehicles.json", { cache: cacheMode }, 10000)
    .then((response) => {
      if (!response.ok) throw new Error(`载具数据加载失败：${response.status}`);
    ...
    .then((vehicleData) => {
      ...
      applyView();
      if (activeTab === "vehicles") hideLoadingPanel();
    })
    .catch((error) => {
  ...
  const loadWeapons = fetchWithTimeout("data/weapons.json", { cache: cacheMode }, 10000)
    .then((response) => {
      if (!response.ok) throw new Error(`枪械数据加载失败：${response.status}`);
      return response.json();
    })
    .then((weaponData) => {
      ...
      applyView();
      if (activeTab === "weapons") hideLoadingPanel();
    })
    .catch((error) => {
  ...
  const loadMaps = fetchWithTimeout("data/maps.json", { cache: cacheMode }, 10000)
    ...
    .then((mapData) => {
      ...
      dataReady.maps = true;
      applyView();
      if (activeTab === "maps") hideLoadingPanel();
    })
    .catch((error) => {
  ...
  const loadModels = fetchWithTimeout("model/models.json", { cache: cacheMode }, 10000)
    .then((response) => {
      if (!response.ok) throw new Error(`模型数据加载失败：${response.status}`);
      return response.json();
    })
    .then((modelData) => {
      ...
      applyView();
      if (activeTab === "models") hideLoadingPanel();
    })
    .catch((error) => {
...
function parsePlayerStreamText(text) {
  ...
  if (!lines.length) throw new Error("玩家数据格式错误：玩家流为空");
...
async function readPlayerStream(response) {
  ...
  try {
    ...
    if (!headerParsed) throw new Error("玩家数据格式错误：玩家流为空");
...
async function loadPlayersInBackground() {
  playersLoading = true;
  playersLoadError = "";
  if (activeTab === "players") applyView();
  try {
...
function shareFactionColors(faction) {
  if (faction === "德军") return { badgeFill: "#ead7bd", badgeColor: "#6d4e2a" };
  if (faction === "美军") return { badgeFill: "#dde8fb", badgeColor: "#3a5ba0" };
  if (faction === "日军") return { badgeFill: "#f3d4ce", badgeColor: "#8a3128" };
  if (faction === "英军") return { badgeFill: "#d9e9dc", badgeColor: "#2c5a3e" };
  if (faction === "通用" || faction === "公用") return { badgeFill: "#fbf1a4", badgeColor: "#6a6320" };
...
function renderTable(list) {
  ...
  rowsEl.innerHTML = list.map((vehicle) => {
    ...
    if (selectedSet.has(index)) cardClasses.push("selected");
    if (targetMode && isUnarmedVehicle(vehicle)) cardClasses.push("no-target-weapon");
...
function renderWeaponTable(list) {
  ...
  weaponRowsEl.innerHTML = list.map((weapon) => {
    const cardClasses = ["asset-card"];
    if (selectedSet.has(weapon.id)) cardClasses.push("selected");
...
targetModal.addEventListener("click", (event) => {
  if (event.target === targetModal) closeDialog(targetModal);
});
closeMapModal.addEventListener("click", () => closeDialog(mapModal));
mapModal.addEventListener("click", (event) => {
  if (event.target === mapModal) closeDialog(mapModal);
});
...
async function checkAssetManifest() {
  ...
  if (refreshedShell === currentShell) session.removeItem(refreshedKey);
...
checkAssetManifest()
  ...
  .then(() => {
    if (vehicles.length === 0 && weapons.length === 0 && maps.length === 0 && models.length === 0 && players.length === 0) return;
    applyView();
  })
  .catch((error) => {
    console.error(error);
