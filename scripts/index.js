...
const weaponSections = [
    {
        title: "基本信息",
        detailFields: ["类型", "致死", "射击间隔", "弹速", "射速", "弹容", "总装填时间", "单次装填时间", "速度修正", "视野修正", "缩圈速率"],
        compareFields: ["致死", "弹速", "射速", "弹容", "总装填时间", "单次装填时间", "速度修正", "视野修正", "缩圈速率"]
    },
    {
        title: "精度",
...
async function loadDataIncremental() {
    ...
    const loadAmmoBlastPower = fetchWithTimeout("data/ammo-blast-power.json", { cache: cacheMode }, 10000)
        .then((response) => {
        if (!response.ok)
            throw new Error(`弹药爆炸威力数据加载失败：${response.status}`);
        return response.json();
    })
        .then((blastPowerData) => {
        if (!Array.isArray(blastPowerData))
            throw new Error("弹药爆炸威力数据格式错误：根节点必须是数组");
    ...
    try {
        while (true) {
            ...
            while (newlineIndex >= 0) {
                ...
                if (line)
                    pendingPlayers.push(JSON.parse(line));
        ...
        if (pendingPlayers.length)
            appendPlayers(pendingPlayers, true);
        else
            playerRankings = buildPlayerRankings(players);
        currentPlayerList = players;
        filteredPlayerList = players;
...
function formatPlayerValue(player, key, formatter) {
    ...
    if (formatter === "distance")
        return value === null || value === undefined ? "-" : `${fmt(Number(value))} km`;
    if (key === "kd_ratio")
        return value === null || value === undefined ? "-" : fmt(Number(value));
...
function weaponFieldDisplay(weapon, key) {
    if (key === "致死")
        return weaponKillDisplay(weapon);
    if (key === "射击间隔") {
...
function getSortedWeapons(list) {
    if (!weaponSortState.key || !weaponSortState.mode)
        return list.slice();
...
function getSortedPlayers(list) {
    if (!playerSortState.key || !playerSortState.mode)
        return list.slice();
...
function compareArrow(left, right) {
    ...
    if (left < right)
        return { left: "↓", right: "↑", leftClass: "down", rightClass: "up" };
...
function compareSymbol(row, side) {
    const comparable = row.comparable !== false;
    if (!comparable)
        return { symbol: "", className: "hidden" };
...
async function renderComparisonCanvas(comparison) {
    ...
    if (!leftIconLoaded)
        drawCenteredText(ctx, "-", layout.leftX + layout.colWidth - 124, cardTop + 28, 92, 92, { font: "900 32px sans-serif", color: "#8a8275", maxLines: 1 });
    if (!rightIconLoaded)
        drawCenteredText(ctx, "-", layout.rightX + layout.colWidth - 124, cardTop + 28, 92, 92, { font: "900 32px sans-serif", color: "#8a8275", maxLines: 1 });
    let y = rowsTop;
    comparison.rows.forEach((row, index) => {
        y += drawShareRow(ctx, row, y, layout);
        if (index < comparison.rows.length - 1)
            y += rowGap;
    });
    return canvas;
}
...
async function renderDetailCanvas(detail) {
    ...
    detail.sections.forEach((section, sectionIndex) => {
        ...
        if (sectionIndex < detail.sections.length - 1)
            y += sectionGap;
    });
    return canvas;
}
...
async function buildDialogSnapshotImage(dialogRoot, preparedCapture = null) {
    ...
    if (!capture)
        throw new Error("Share dialog target is not available");
    ...
    try {
        ...
        if (!blob)
            throw new Error("Dialog snapshot export failed");
        return blob;
    }
    finally {
...
async function openShareDialog(sourceDialogRoot = null) {
    ...
    if (shareTitle)
        shareTitle.textContent = `分享${shareDialogTitle(sourceDialogRoot)}`;
...
function downloadCurrentShareImage() {
    const sharePayload = currentDetailShare || currentComparison;
    if (!shareImageBlob || !shareImageUrl || !sharePayload)
        return;
...
function renderPlayerPagination(totalPages) {
    ...
    ${items.map((item) => item === "..."
        ...
        : `<button class="page-button ${item === playerPage ? "active" : ""}" data-page="${item}" type="button">${item}</button>`).join("")}
...
function getThreatFactors(vehicle) {
    ...
    if (weapon === "—")
        return factors([]);
    if (vehicle.name === "Sd.Kfz.251/17防空车" || vehicle.name.includes("山猫"))
        return factors(lightFlakThreats);
    if (vehicle.name === "鱼雷艇")
        return factors(boforsThreats);
    if (vehicle.name === "M3“斯图亚特”轻型坦克")
        return factors(stuartCannonThreats);
    if (vehicle.name === "九五式轻战车" || vehicle.name === "装甲艇")
        return factors(lightCannonThreats);
    if (vehicle.name === "特四式内火艇")
        return factors(katsuMgThreats);
    if (vehicle.name === "三号突击炮" || vehicle.name.includes("萤火虫") || vehicle.name.includes("TOG") || vehicle.name.includes("狼獾"))
        return factors(tankDestroyerThreats);
    if (weapon.includes("重机枪") || weapon.includes("机枪") || weapon.includes("喷火器"))
        return factors(mgThreats);
    if (weapon.includes("无后坐力"))
        return factors(recoillessThreats);
    if (weapon.includes("88mm") || weapon.includes("128mm"))
        return factors(heavyCannonThreats);
...
function setActiveTab(tab) {
    ...
    if (tab === "players")
        playerPage = 1;
    applyView();
    if (tab === "maps" || tab === "models" || tab === "players")
        resetTablePosition();
    if (tab === "maps")
        warmMapAssets();
}
...
async function loadMapData(mapId) {
    ...
    if (!summary)
        return null;
    if (mapDataCache.has(mapId))
        return mapDataCache.get(mapId);
...
function mapImageNames(faction) {
    ...
    add(faction?.image);
    if (faction?.id)
        add(`map_${String(faction.id).toLowerCase()}.webp`);
    if (faction?.id === "Axis")
        add("map_axis.webp");
    if (faction?.id === "Allies")
        add("map_allies.webp");
    if (faction?.id === "USMC")
        add("map_usmc.webp");
    if (faction?.id === "IJA")
...
function ownerLabelForState(factionId, state, factions) {
    if (state === "friendly_all")
        return (factions.find((item) => item.id === factionId) || factionMeta(factionId) || {}).label || factionId;
    if (state === "enemy_all")
        return enemyFactionFor(factionId, factions).label;
    return "";
}
...
function placeThreat(item, x, y) {
    ...
    if (!isThreatPositionClear(nextX, nextY, size, item)) {
        ...
        activeThreats.forEach((other) => {
            if (other === item)
                return;
            candidates.push({ x: other.x - step, y: other.y }, { x: other.x + step, y: other.y }, { x: other.x, y: other.y - step }, { x: other.x, y: other.y + step }, { x: other.x - step, y: other.y - step }, { x: other.x + step, y: other.y - step }, { x: other.x - step, y: other.y + step }, { x: other.x + step, y: other.y + step });
        });
...
function threatScore(item, geometry = getTargetGeometry()) {
    ...
    const maxDistance = Math.max(Math.hypot(barrelTip.x, barrelTip.y), Math.hypot(stageRect.width - barrelTip.x, barrelTip.y), Math.hypot(barrelTip.x, stageRect.height - barrelTip.y), Math.hypot(stageRect.width - barrelTip.x, stageRect.height - barrelTip.y)) || 1;
...
function renderPriorityList(items, topTag, allowReorder = true) {
    ...
    priorityList.querySelectorAll(".priority-item").forEach((node) => {
        if (!activeThreats.has(node.dataset.tag))
            node.remove();
    });
    Array.from(priorityList.children).forEach((node, index) => {
    ...
    Array.from(priorityList.children).forEach((node, index) => {
        ...
        if (!firstRect || previousRanks.get(node.dataset.tag) === index)
            return;
...
function weaponSectionFields(section, mode) {
    if (mode === "compare" && section.compareFields)
        return section.compareFields;
    if (mode === "detail" && section.detailFields)
...
function handleMapPerspectiveClick(perspectiveButton) {
    ...
    loadMapData(perspectiveButton.dataset.mapId).then((mapData) => {
        if (mapData)
            return openProcessedMap(perspectiveButton.dataset.faction, mapData, popup, perspectiveButton.dataset.state || "friendly_all");
        showPopupError(popup, "地图图片生成失败", "请关闭这个页面后，回到主页面重新点击阵营视角按钮生成一次。");
    }).catch((error) => {
        console.error(error);
        showPopupError(popup, "地图图片生成失败", "请关闭这个页面后，回到主页面重新点击阵营视角按钮生成一次。");
