...
玩家列表页会显示 `ID - XP - 击杀数 - 死亡数 - K/D - 最长连杀`，ID 以浅蓝色药丸徽章显示，支持按数值列全量升降排序，每页显示 100 个玩家。列表下方有独立分页容器，分页控件在容器中水平和垂直居中。点击玩家行会打开详情弹窗，顶部先显示大号玩家名卡片，再显示排名卡片，并显示 XP、击杀、死亡、分数、KD、游戏时间、最长连杀、摧毁目标、摧毁车辆、治疗士兵、误伤、移动距离、开火次数、投掷物次数在当前快照所有玩家中的排名。
