...
模型查看页在 `720px` 以下会显示“操作说明”按钮，点击后展开手势说明抽屉；光照滑动条固定在页面底栏，避免占用模型主要显示区域；模块显示入口放在操作说明下方，点击“模块显示”后从右侧滑出窄侧栏，侧栏中的模块按钮使用 `1`、`2`、`3` 这样的数字标识以节省空间。手机端说明使用触控逻辑和手机图标：单指滑动屏幕查看模型，双指张合缩放模型，双指同时拖动移动视角。`420px` 以下会进一步压缩标题、光照控件和部件按钮宽度，适配更窄屏幕。
