...
这是一个用于浏览和对比 Running With Rifles DLC 数据的纯前端静态工具。页面可以查询枪械参数、载具参数、地图信息、地图设施点位、载具模型和太平洋玩家统计，并支持卡片浏览、搜索、详情弹窗、双对象对比、索敌优先级计算、玩家统计排序、生成带设施图标的地图视图，以及在独立页面中查看 GLB 模型。
...
载具查询使用和模型查询一致的卡片式布局。每张卡片显示阵营徽章、载具图标和载具名，支持搜索、详情、对比和索敌优先级工具；点击卡片会打开载具详细参数，比较模式下点击卡片会选择两个载具进行对比，索敌优先级模式下点击有武器的载具会打开索敌优先级弹窗。

枪械查询使用和模型查询一致的卡片式布局。每张卡片显示阵营徽章、顺时针旋转 90 度后的枪械图标和枪械名称，支持搜索、详情和对比；点击卡片会打开枪械详细参数，比较模式下点击卡片会选择两个枪械进行对比。枪械查询页不提供排序控件。
...
主页面当前使用暖纸张风格：米色纸面背景、墨色文字、浅蓝批注块、浅黄色当前标签和低对比手绘边框。维护 `index.html` 的样式时，优先保留这套低饱和、偏纸张质感的视觉语言，不要重新混入高饱和渐变或玻璃拟态效果。

阵营徽标需要保留明确区分：美军浅蓝色、德军浅棕色、英军浅绿色、日军浅红色、通用浅黄色。载具和枪械比较模式中，被选中的卡片或行只使用绿色描边/绿色淡底；玩家列表 hover 只做轻微浅黄底反馈，不使用红色横向或侧向描边。
